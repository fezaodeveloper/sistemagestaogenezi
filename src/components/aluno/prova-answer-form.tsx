...
import type { SubmeterProvaState } from "@/app/aluno/cursos/[id]/modulos/[moduloId]/prova/actions";
...
export function ProvaAnswerForm({
  action,
  questoes,
  submitLabel,
}: {
  action: (state: SubmeterProvaState, formData: FormData) => Promise<SubmeterProvaState>;
  questoes: QuestaoParaResponder[];
  submitLabel: string;
}) {
  const [state, formAction] = useActionState<SubmeterProvaState, FormData>(action, undefined);
