...
import {
  ...
  notificarContratoPendente,
  notificarLeadSemContato,
...
async function executarHandler(tipo: string, payload: EventoPayload): Promise<void> {
  switch (tipo) {
    ...
    case "contrato.assinado":
      await notificarContratoAssinado(payload);
      return;
    case "contrato.pendente":
      await notificarContratoPendente(payload);
      return;
    case "parcela.vencendo.amanha":
      await notificarParcelaVencendoAmanha(payload);
      return;
    case "lead.sem.contato":
      await notificarLeadSemContato(payload);
      return;
    case "resgate.novo":
      await notificarNovoResgate(payload);
      return;
    case "resumo.mensal":
    ...
    case "empresa.cadastro":
      await notificarEmpresaCadastro(payload);
      return;
    ...
    case "conecta.assinatura.cancelada":
      ...
      return;
    case "conecta.vaga.nova":
      await notificarNovaVagaConecta(payload);
      return;
    case "resumo.diario":
    ...
    case "agendamento.criado":
      await notificarAgendamentoCriado(payload);
      return;
    case "lembretes.agendamentos.resumo":
      await notificarLembretesAgendamentosResumo(payload);
      return;
