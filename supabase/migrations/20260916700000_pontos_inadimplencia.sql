-- Bloqueio de pontuação para alunos inadimplentes (PARTE 2 da tarefa
-- "pontos_inadimplencia") — reescreve as mesmas 3 functions já recriadas em
-- 20260909300000_limite_pts_dia.sql (marcar_aula_concluida,
-- criar_tentativa_quiz, criar_tentativa_prova), adicionando uma verificação
-- de inadimplência que zera os pontos calculados (a ação em si continua
-- sendo registrada normalmente — aula concluída, tentativa de quiz/prova
-- salva — só o INSERT em pontos_eventos sai com 0).
--
-- Verificação direta em parcelas.aluno_id = auth.uid() (a tabela já tem
-- aluno_id próprio, não precisa passar por matriculas — ver
-- 20260911100000_modulo_financeiro.sql). Sem chamada ao Asaas: usa só o
-- status já sincronizado no banco pelo cron de atraso existente.
--
-- Aplicado como ÚLTIMA verificação, depois do cálculo normal de pontos e
-- depois do teto diário — a mesma variável v_inadimplente é calculada uma
-- única vez no topo de cada function e reaproveitada em todos os pontos de
-- INSERT daquela function (marcar_aula_concluida tem 3: aula, módulo,
-- curso).
--
-- Nome do arquivo ajustado de 20260911100000 (pedido original) para
-- 20260916700000: 20260911100000_modulo_financeiro.sql já existe.
--
...
begin
  ...
  -- Inadimplência — calculada uma vez, usada nos 3 pontos de INSERT abaixo.
  select exists (
  ...
  v_pontos_calcular := case when v_inadimplente then 0 else v_pontos_calcular end;
...
begin
  ...
  -- Inadimplência — última verificação, depois do teto diário.
  select exists (
    select 1 from public.parcelas p
    where p.aluno_id = auth.uid()
    and p.status = 'atrasado'
